...
export async function handleRequest(request: Request): Promise<Response> {
  ...
  if (Route.match(url)) {
    if (Route.isAuth(url)) return auth(Config.client, Config.scope, DriveAuthType.CODE, Config.redirect)
...
async function link(path: string): Promise<Response> {
  ...
  if (json) {
    ...
    try {
      ...
      const { "@microsoft.graph.downloadUrl": href, file: { mimeType: type }} = data
      const origin = await fetch(href,{ headers: { authorization } })
      return new Response(origin.body, { headers: { ...res.headers, ...Config.corsHeaders, "Content-Type": type }})
    } catch (error) {
  ...
  async function auth(clientId: string, scope: string[], responseType: DriveAuthType, redirectUri: string): Promise<Response> {
  ...
  async function call(url: URL): Promise<Response> {
    ...
    const link = new URL('https://login.microsoftonline.com/common/oauth2/v2.0/token')
